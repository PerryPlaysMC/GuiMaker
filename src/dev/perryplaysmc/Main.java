...
public class Main extends JavaPlugin {
    ...
    public boolean onCommand(CommandSender s, Command command, String cl, String[] args) {
        ...
        if(type == "default") {
        ...
        } else if(getConfig().getConfigurationSection("formats." + type) != null) {
            ...
            ConfigurationSection sec = getConfig().getConfigurationSection("formats." + type);
            int itemIndex = 0;
            for(int i = 0; i < c.getInventory().getContents().length; i++) {
                ItemStack item = c.getInventory().getItem(i);
                if(item != null) {
        ...
        contents.add("}");
        ConfigManager.printToFile(contents, new File(getDataFolder(), "inventory-" + type + "-" + index + ".java"));
